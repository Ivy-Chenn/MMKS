...
1. Check whether the app is the latest version.
2. Connect to the phone hotspot and try the upgrade to see whether it succeeds.
...
1. Check whether the "Reset Base Station Position" option was selected by mistake.
2. If so, after the base station position is reset, the map needs to be re-drawn.
...
1. Verify whether a backup map exists. If the RTK base station position has not moved and a backup map exists, the map can be restored. Otherwise, the map needs to be re-drawn.
2. Restart the device and the RTK base station.
...
1. Verify whether the issue is caused by the network. Test whether the upgrade succeeds when connecting to the phone's hotspot.
...
2. Confirm that the network signal is good.
